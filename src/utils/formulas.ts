...
export function evaluateFormula(formula: string, getCellValue: (row: number, col: number) => CellValue): number | string | boolean | null{
    ...
    if (addMatch) {
        const pos1 = idToCell(addMatch[1]);
        const pos2 = idToCell(addMatch[2]);
        if (pos1 && pos2) {
            const val1 = getCellValue(pos1.row, pos1.col);
            const val2 = getCellValue(pos2.row, pos2.col);
    ...
    const mulMatch = expr.match(/^([A-Z]+[0-9]+)\*([0-9]+)$/);
    if (mulMatch) {
